...
void loop() {
    ...
    // readByte(0x00);
    uint8_t size = 4;
    uint8_t arr[size] = {0, 0, 0, 0};
    getSecs(readByte(0x00), arr);
    getMins(readByte(0x01), arr);
    drawNumbers(arr, size, clkDiv);
 ...
 */
// TODO; Make  size an enum that defines either 4 or 4 digit time
 ...
 * @param bit - 0 or 1
 */
void writeBit(const uint8_t reg, uint8_t pos, const uint8_t bit) {
    switch (bit) {
        case 0: {
